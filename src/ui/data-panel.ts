...
const hashInputs = (i: HistogramInputs): string => {
    ...
    return `${i.splatId}|${i.prop}|${m}|${i.onScreenOnly ? 1 : 0}|${i.logScale ? 1 : 0}|` +
...
class DataPanel extends Container {
    constructor(events: Events, tooltips: Tooltips, args = { }) {
        ...
        const populateDataSelector = (splat: Splat) => {
            ...
            const extras: any = {
                ...(!onScreenOnlyValue.value && {
                    nx: `${i18n.t('panel.splat-data.normal')} X`,
                    ny: `${i18n.t('panel.splat-data.normal')} Y`,
                    nz: `${i18n.t('panel.splat-data.normal')} Z`
                }),
            ...
            // retain custom PLY columns from the design branch. properties not
            // represented by the engine's packed GPU textures use the raw CPU
            // fallback below.
            const listedProps = new Set([...defaultProps, ...extraProps, 'state', 'transform']);
            const remainingProps = showAllValue.value && !onScreenOnlyValue.value ?
            ...
            if (allProps.length > 0 && !allProps.includes(selectedDataProp)) {
                ...
                inputs.prop = selectedDataProp;
                inputs.mode = propModeFor(selectedDataProp) ?? -1; // eslint-disable-line no-use-before-define
            }
            ...
            allProps.forEach((prop) => {
                ...
                item.addEventListener('click', () => {
                    ...
                    // eslint-disable-next-line no-use-before-define
                    inputs.prop = prop;
                    inputs.mode = propModeFor(prop) ?? -1; // eslint-disable-line no-use-before-define
        ...
        const calcRawHistogram = (target: Splat, prop: string) => {
            const values = target.splatData.getProp(prop) as ArrayLike<number>;
            const state = target.splatData.getProp('state') as Uint8Array;
            const selected = new Float32Array(256);
            const unselected = new Float32Array(256);
            if (!values || !state) {
            ...
            for (let i = 0; i < state.length; i++) {
                ...
                const bucket = Math.min(255, Math.floor(n * 256));
                (state[i] === 1 ? selected : unselected)[bucket]++;
            }
        ...
        const scheduleUpdate = () => {
            ...
            splat.scene.commandQueue.enqueue(async () => {
                ...
                try {
                    const result = mode < 0 ?
                        calcRawHistogram(splat, prop) :
                        await splat.scene.dataProcessor.calcHistogram(splat, mode, opts);
                    if (myToken !== pendingToken) return;
        ...
        events.on('selection.changed', (selection: Element) => {
            if (selection instanceof Splat) {
                ...
                inputs.prop = selectedDataProp;
                inputs.mode = propModeFor(selectedDataProp) ?? -1;
                populateDataSelector(splat);
                tick();
            }
        });
        ...
        const showStats = (count: number, selected: number, total: number) => {
            ...
            const fmt = (n: number) => i18n.formatInteger(n);
        ...
        histogram.events.on('select', (op: string, start: number, end: number) => {
            ...
            targetSplat.scene.commandQueue.enqueue(async () => {
                if (mode < 0) {
                    const values = targetSplat.splatData.getProp(prop) as ArrayLike<number>;
                    const state = targetSplat.splatData.getProp('state') as Uint8Array;
                    const data = new Uint8Array(state.length);
                    for (let i = 0; i < state.length; i++) {
                        if (state[i] !== 0 && state[i] !== 1) continue;
                        const value = values[i];
                        if (!Number.isFinite(value)) continue;
                        const n = minValue === maxValue ? 0 : (value - minValue) / (maxValue - minValue);
                        const bucket = Math.min(numBins - 1, Math.floor(n * numBins));
                        if (bucket >= start && bucket <= end) data[i] = 255;
                    }
